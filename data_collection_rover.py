...
def main():
    ...
    parser.add_argument(
        "--three-bars",
        action="store_true",
        help="Display only three bars (left, right, up) instead of four."
    )
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_s:
                        if time.time() >= start_enable_time:
                            ...
                            active_directions = list(base_directions) # Copy base
                            if args.three_bars:
                                if "down" in active_directions: active_directions.remove("down")
